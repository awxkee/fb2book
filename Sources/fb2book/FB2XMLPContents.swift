...
extension FB2XMLPContents: Codable {
    public enum CodingKeys: String, XMLChoiceCodingKey {
        case image
        case strong
        case emphasis
        case a
    ...
    public init(from decoder: Decoder) throws {
        ...
        if container.contains(.emphasis) {
            let str = try container.decode(String.self, forKey: .emphasis)
            self = .strong(str)
            return
        }
        if container.contains(.a) {
            let str = try container.decode(String.self, forKey: .a)
